...
//            IOIndexed.writeIndexed(ImageManipulation.MatrixToImage(newImage, myPicture.getWidth(),myPicture.getHeight(),myPicture.getType()), nColors, String.format("output(%d).rii", i++));
//        }
//        for (int i=0;i<22;i++){
...
//        ImageIO.write(ImageManipulation.MatrixToImage(newImage2.pixels, newImage2.width,newImage2.height,myPicture.getType()), "jpg", new File("new-test2.jpg"));
        ...
        Searcher.setTarget("drive2.rii");
        File[] fff = Searcher.Search("./riitests");
        int i=0;
        for (File ff: fff){
            IndexedImage newImage2 = IOIndexed.readIndexed(ff.getAbsolutePath());
            ImageIO.write(ImageManipulation.MatrixToImage(newImage2.pixels, newImage2.width,newImage2.height,5), "jpg", new File(String.format("output/888new-test(%d).jpg", i++)));
        }
