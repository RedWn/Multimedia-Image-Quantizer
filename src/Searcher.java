...
public class Searcher {
    ...
    public static boolean areColorsClose(double[] c1, double[] c2) {
        return Math.sqrt(Math.pow(c1[0] - c2[0], 2) + Math.pow(c1[1] - c2[1], 2) + Math.pow(c1[2] - c2[2], 2)) < color;
    }

    public static boolean areColorsClose(double[] c1, double[] c2,double threshold) {
    ...
    public static boolean arePercentagesClose(float c1, float c2) {
        return Math.abs(c1 - c2) < per;
    }

    public static float getAllSimilarColorsPercentages(Vector<Pixel> colors,Vector<Float> percentages,int index){
        float ans = 0;
